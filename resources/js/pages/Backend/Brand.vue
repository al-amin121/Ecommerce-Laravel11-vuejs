...
const file = e.target.files[0];
editAblebrand.value.image = file; 
...
try{
 const response = await sendRequest({
        method: 'POST',
        url: `api/Brand/${id}?_method=PUT`,
        headers: {'Content-Type': 'multipart/form-data'},
        data: editAblebrand.value,
    });
    resetModel()
    getData()
    if (response.data) {
   ...
   <AppLayout>
    <div class="mx-5 mt-5">
        <div class="flex justify-start">
            <h2 class="font-sans font-semibold text-3xl">Brands</h2>
        </div>
        <div class="flex mt-5 ">
            <div class="grow  p-5 pb-16">
                <ul>
                    <li v-for="brand in brands" :key="brand.id"   class="mb-1">
                        <div class="bg-white flex shadow-md shadow-primary rounded-md p-2 h-20  content-center">
                            <div class="grow flex justify-center w-20">
                                <img :src="brand.image"  class="w-16 h-16 rounded-full">
                            </div>
                            <div class="grow ms-2 flex justify-start items-center font-sans font-medium text-lg">{{brand.name}}</div>  
                            <div class="grow flex justify-end items-center gap-5 pe-3">
                                ...
                                <button @click="deleteData(brand.id)" class="flex justify-center text-red-950 bg-red-200  w-12 h-7 p-1 rounded-full hover:text-red-950 hover:bg-red-300">
             ...
             <div class="grow pe-2 mb-20"> 
                <div class="p-5 w-96 mx-auto shadow-md shadow-primary rounded-md bg-white"> 
                    <div class="form-item h-20 mt-2 ">
                        <label for="name" class="w-48 font-sans text-xl">Brand Name:</label>
                        <input v-model="form.name" type="text" class="mt-2 w-full font-sans h-10 rounded-md border border-gray-400 p-3" name="name" id="name"  placeholder="Enter Brand Name" />
                    </div> 
                    <div class="mt-2 mb-5">
                        <div  >
                            <label for="brand-image" class=" flex w-full h-28 justify-center items-center rounded-md border-dashed border-2 border-primary bg-white text-primary">
                    ...
                    <div class="pb-5">
                        <button v-on:click="addBrand"  type="submit" class="flex justify-center items-center font-sans p-2 rounded-3xl w-44 h-10 bg-primary hover:bg-purple-900 text-white"
                            :disabled="isLoading"
                            >
        ...
        <div  v-if="isModel"   class="model fixed  z-50 inset-0 bg-gray-900 bg-opacity-60 overflow-y-auto h-full w-full ">
            <div class="relative top-40 mx-auto shadow-xl rounded-md bg-white w-1/3">
                ...
                <div class="">
                    ...
                    <div class="form-item h-20 mt-2 ">
                        <label for="name" class="w-48 mb-2 font-sans text-xl items-start flex justify-start">Brand Name:</label>
                        <input v-model="editAblebrand.name" type="text" class="w-full font-sans h-10 rounded-md border border-gray-400 p-3" name="name" id="name"  placeholder="Enter Category Name" />
                    </div> 
                    <div class="mt-2 ">
                        <div class="flex" >
                            <label for="brand-update" class=" flex w-full h-32 justify-center items-center rounded-md border-dashed border-2 border-primary bg-white text-primary">
                            ...
                            <div  class="h-32 ms-5 w-full rounded-md flex justify-center items-center border-dashed border-2 border-primary bg-white text-primary">
                                ...
                                <img v-else :src="editAblebrand.image" class=" h-32 w-full bg-cover rounded-md p-1 ">         
                            </div>
                        </div>
                    </div>
                    <div class="mt-5 pb-5">
                        <button @click="updateForm(editAblebrand.id)" 
